<template>
  <section class="group-manage">
    ...
    <el-table
      ...
      <el-table-column align="center" prop="code" label="组编码" />
...
<script>
import { getGroupsByPage, addGroup, editGroup, deleteGroup } from "@assets/js/api/group";
export default {
  name: "group",
  components: {},
  props: {},
  ...
  methods: {
    searchByPage() {
      ...
      getGroupsByPage(this.currentPage, this.pageSize, this.search.keyword).then(
        (res) => {
          console.log(res);
          if (res.code === "200") {
    ...
    toDelete(row) {
      this.$confirm(`确认删除名为【${row.name}】的组?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteGroup(row.businessId).then( res => {
          if (res.code === "200") {
            this.$message.success("组删除成功");
            this.refreshSearch();
          }
        })
      })
